...
ChooseRef::ChooseRef(unsigned num_ref, Clu_Point &cluster, Point &main_pivot,vector<Point> &oth_pivot,int type)
{
    ...
    this->dim = cluster.clu_point[0].coordinate.size();  //cluster每一簇的数据，main_pivot是center，oth_pivot是pivot
    ...
    if(type == 0)
        ...
        // by input
        this->RefPoint_Set = ChooseRefPoint_Input(cluster,oth_pivot); //这一步是计算当前cluster的下所有pivot与cluster所有数据点的距离，然后拟合出一个模型
    this->mainRefDisArr = CaculateDisArr(cluster, mainRefPoint); // mainRefPoint是center，这是要计算当前cluster下center到所有数据点的距禋
    // this->r_mainRefPoint = CaculateR(cluster, mainRefPoint);
    this->main_pointSet = mainRef_Point(mainRefPoint, mainRefDisArr[num_data - 1], mainRefDisArr[0], RefPoint_Set); //是要得到cluster的上界和下界，以及每个pivot的上界和下界
    this->main_pointSet.setMainRefDisArr(mainRefDisArr);
    vector<double> mainRefPt_coeffs = TrainModel(main_pointSet.dis, Pos); //文中没提到要对center到所有数据点的距离进行拟合
...
vector<Ref_Point> ChooseRef::ChooseRefPoint_Input(Clu_Point &cluster,vector<Point> &oth_pivot){
    ...
    for(unsigned i = 0; i < num_ref; i++){
        ...
        ref_point.setDisArr(dis);  //把数据到支撑点的距离存到对应支撑点下
        // ref_point.setDict(CaculateCircleBound(ref_point.dis));

        vector<double> coeffs = TrainModel(ref_point.dis, Pos); //ref_point.dis是距离值,Pos是数据的真实rank,coeffs是拟合的系数
        ref_point.setCoeffs(coeffs);
        // cout << "ref " << i << " RMSE : " << CaculR2(ref_point.dis, coeffs) << ",";

        ref_points.push_back(ref_point);   // ref_point相当于一个pivot table，存了当前的pivot和对应数据的距离，以及拟合的系数
    }
    return ref_points;
}

vector<double> ChooseRef::CaculateDisArr(Clu_Point &cluster, Point &refPoint){
    vector<double> dis;  //refPoint是center
    for(int i = 0; i < num_data; i++){
