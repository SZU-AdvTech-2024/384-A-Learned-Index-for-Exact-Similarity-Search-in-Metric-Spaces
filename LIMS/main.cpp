...
int main(int argc, const char* argv[]){
    ...
    // Pretreatment : clustering and cluster evaluation(用来评估选出来的centriod是否合适)
    ...
    if(flag_eva){
        ...
        string data_path = "home/root_mly/datasets/LIMS/dataset_LIMS/Imagenet32_train/color_32.txt";//home/root_mly/datasets/LIMS/dataset_LIMS/Imagenet32_train // /home/qrstu/yty/dataset/color/color_32.txt
        for(unsigned i = min_clu; i < max_clu + 1; i += step){
            ...
            string clu_data_path = "/home/root_mly/PProject/LIMS/LIMS/outputFiles/K_" + to_string(i); // /home/root_mly/PProject/LIMS/LIMS/inputFiles
    ...
    // all data
    vector<Clu_Point> all_data; //聚类数据
    vector<mainRef_Point> all_refSet; //支撑点数据
    ...
    string filename = "/mnt/e/dataset_LIMS/clu100_pivot3/centers.txt"; //  原路径：/home/root_mly/PProject/LIMS/LIMS/inputFiles
    ifstream fin;
    fin.open(filename);
    if(!fin){
        cout << filename << " file could not be opened\n";
    ...
    while(getline(fin,line)){
        ...
        pivots.push_back(pivot_pt);   //pivot存放的是cluster的center（？）
    }
    fin.close();


    // 读取聚类的center(?)   /inputFiles里的ref_p对应的是第p个ref_,即每个cluster的centroid，但为什么每个ref_有两行
    // 亦或者是读取每一个cluster的pivot
    for(unsigned p = 0; p < num_clu; ++p){
        string filename = "/mnt/e/dataset_LIMS/clu100_pivot3/selected/selected_" + to_string(p) +".txt"; // /home/root_mly/PProject/LIMS/LIMS/inputFiles
        ifstream fin;
        fin.open(filename);
        if(!fin){
            cout << filename << " file could not be opened\n";
    ...
    for(unsigned i = 0; i < num_clu; i++){
        InputReader inputReader("/mnt/e/dataset_LIMS/clu100_pivot3/clusters/cluster_" + to_string(i) + ".txt");  // /home/root_mly/PProject/LIMS/LIMS/inputFiles
        ...
        // all_data[i]是第i个cluster，pivots[i]是第i个cluster的center，oth_pivots[i]是第i个cluster的pivots, ref_point.mainRefPoint是center，ref_point.RefPoint_Set是pivot集
        chrono::steady_clock::time_point begin = chrono::steady_clock::now();
        ChooseRef ref_point(num_ref - 1, all_data[i], pivots[i],oth_pivots[i],1); // 索引构建部分的主要部分，对应原文中的Fig1(b):通过计算pivot到数据的距离，得到相应的rank，并用来训练RP model
        CaculateIValue calIValue(all_data[i], ref_point.getMainRefPoint()); // Page？LIMS Value? ref_point.getMainRefPoint()就是main_pointSet，包括了center和pivot的rank和距离
    ...
    filename = "/mnt/e/dataset_LIMS/query/queries.txt"; ///home/root_mly/PProject/LIMS/LIMS/inputFiles  //"/home/qrstu/yty/LIMS/inputFiles/KNN.txt"
    ...
    // range query
    string rangeQuery_filename = "/mnt/e/dataset_LIMS/query/queries.txt";
    ...
    for(unsigned m = 0; m < list_rangeQry.size(); ++m){
        ...
        for(unsigned i = 0; i < num_clu; ++i){
            CalCirclePos mainRefPtCircle(all_refSet[i].point, all_refSet[i].r, list_rangeQry[m], r); // all_refSet[i]是第i个cluster的center
            if(mainRefPtCircle.label == 1){
                continue;
            }
            if(mainRefPtCircle.label == 3 && mainRefPtCircle.dis_upper < all_refSet[i].r_low){
